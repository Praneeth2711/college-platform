...
export default function CollegeDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (error || !college) return (
    <div className="max-w-4xl mx-auto px-4 py-20 text-center">
      ...
      <h2 className="text-white/60 text-xl font-medium mb-2">College not found</h2>
      <Link href="/" className="text-violet-400 text-sm hover:underline">← Back to colleges</Link>
    </div>
  );
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 pb-16">
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mb-6">
        <Link href="/" className="text-white/40 hover:text-white/70 text-sm flex items-center gap-1 transition-colors">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
        ...
        <div className="relative z-10">
          <div className="flex flex-wrap items-start justify-between gap-4 mb-4">
            ...
            <span className={`px-3 py-1.5 rounded-xl text-xs font-semibold ${college.type === "Government" ? "bg-emerald-500/15 text-emerald-400 border border-emerald-500/20" : "bg-blue-500/15 text-blue-400 border border-blue-500/20"}`}>
              {college.type}
            </span>
          </div>
        ...
        <div className="flex border-b border-white/5">
          {tabs.map((tab, i) => (
            <button key={tab} onClick={() => setActiveTab(i)}
              className={`relative flex-1 py-3.5 text-sm font-medium transition-colors ${activeTab === i ? "text-white" : "text-white/40 hover:text-white/70"}`}>
              {activeTab === i && (
          ...
          <motion.div key={activeTab} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}
            ...
            {activeTab === 0 && (
              <div>
                ...
                <p className="text-white/60 text-sm leading-relaxed">{college.description}</p>
                <div className="mt-6 grid grid-cols-2 gap-4">
                  <div className="bg-white/[0.03] rounded-xl p-4">
                    <p className="text-white/40 text-xs mb-1">City</p>
                    <p className="text-white text-sm font-medium">{college.city}</p>
                  </div>
                  <div className="bg-white/[0.03] rounded-xl p-4">
                    <p className="text-white/40 text-xs mb-1">State</p>
                    <p className="text-white text-sm font-medium">{college.state}</p>
                  </div>
                </div>
              </div>
            )}
            {activeTab === 1 && (
              <div className="space-y-3">
                ...
                {college.courses.map((course) => (
                  <div key={course.id} className="flex items-center justify-between bg-white/[0.03] rounded-xl p-4">
                    <div>
                      <p className="text-white text-sm font-medium">{course.courseName}</p>
                    </div>
                    <span className="text-white/40 text-xs bg-white/5 px-2.5 py-1 rounded-lg">{course.duration}</span>
                  </div>
                ))}
                {college.courses.length === 0 && <p className="text-white/30 text-sm">No courses listed</p>}
              </div>
            )}
            {activeTab === 2 && (
              <div>
                ...
                <div className="space-y-4">
                  <div>
                    <div className="flex justify-between mb-1.5">
                      <span className="text-white/60 text-sm">Placement Rate</span>
                      <span className="text-emerald-400 text-sm font-semibold">{college.placementPercentage}%</span>
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 mt-4">
                    <div className="bg-white/[0.03] rounded-xl p-4 text-center">
                      <p className="text-2xl font-bold text-white mb-1">{college.placementPercentage}%</p>
                      <p className="text-white/40 text-xs">Students Placed</p>
                    </div>
                    <div className="bg-white/[0.03] rounded-xl p-4 text-center">
                      <p className="text-2xl font-bold text-violet-400 mb-1">₹{(college.fees / 100000).toFixed(1)}L</p>
                      <p className="text-white/40 text-xs">Annual Investment</p>
                    </div>
                  </div>
                </div>
              </div>
